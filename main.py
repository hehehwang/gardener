...
def do_routine(csv_file_name: str):
    ...
    with open(csv_file_name, "r") as f:
        ...
        ax2.plot(
            time_lst, [average_humi] * len(time_lst), ":", color="red", label="average"
        )
    ...
    with open(f"doc/{time.strftime('%Y-%m-%d')}.md", "w") as md:
        md.write(
            f"""![some image](https://hehehwang.github.io/gardener/img/{time.strftime('%Y-%m-%d')}.jpg)
    ...
    while 1:
        ...
        while 1:
            ...
            if humi is None or temp is None:
                humi, temp = INVALID_VALUE, INVALID_VALUE
            if prev_humi and HUMI_TOLERANCE < abs(prev_humi - humi):
                continue
            if prev_temp and TEMP_TOLERANCE < abs(prev_temp - temp):
                continue
